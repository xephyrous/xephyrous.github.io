...
`;document.addEventListener("DOMContentLoaded",()=>{u()});document.getElementById("login-gentleman").addEventListener("click",async()=>{if(document.getElementById("login-gentleman").getBoundingClientRect().left<229){console.log(document.getElementById("login-gentleman").getBoundingClientRect().left);return}await m()?setTimeout(()=>{window.location.href="/projects/answers-time/admin-panel/index.html"},500):setTimeout(()=>{window.location.href="/projects/answers-time/login/index.html"},500)});
