import{c as b,g as v}from"./firebase-BMkLKdpF.js";import{a as y}from"./youtube-Bniyde3c.js";b();let u=[];const d={ADD:"../icons/add.png",REMOVE:"../icons/remove.png",NONE:""};document.querySelector("#app").innerHTML=`
...
`;document.getElementById("home-button").addEventListener("click",()=>{window.location="/projects/answers-time/index.html"});document.querySelector('[aria-controls="production-studio"]').addEventListener("click",()=>{const o=document.getElementById("production-studio");o.innerHTML="",g(o,u,d.REMOVE)});document.addEventListener("DOMContentLoaded",async()=>{document.getElementById("main-window").style.display="block";const o=await y(),h=/answering\syour\squestions\s\d/i,r=o.filter(e=>h.test(e.title)).reduce((e,t)=>{const a=parseInt(e.title.trim().slice(-1),10)||0;return(parseInt(t.title.trim().slice(-1),10)||0)>a?t:e},o[0]),s=await v(),c=s.filter(e=>new Date(e.date)>new Date(r.publishedAt)).sort((e,t)=>new Date(t.date)-new Date(e.date)),i=s.filter(e=>!c.some(t=>t.date===e.date)).sort((e,t)=>new Date(t.date)-new Date(e.date));g(document.getElementById("questions"),c,d.ADD),document.querySelector("[aria-controls='questions']").children[0].innerText="Questions ("+c.length+")",g(document.getElementById("questions-archive"),i,d.ADD),document.querySelector("[aria-controls='questions-archive']").children[0].innerText="Questions Archive ("+i.length+")";const n=document.querySelectorAll('[role="tab"]'),l=document.querySelectorAll('[role="tabpanel"]');n.forEach(e=>{e.addEventListener("click",()=>{n.forEach(a=>a.setAttribute("aria-selected","false")),l.forEach(a=>a.hidden=!0),e.setAttribute("aria-selected","true");const t=e.getAttribute("aria-controls");document.getElementById(t).hidden=!1})})});function g(o,h,m){for(const r of h){const s=document.createElement("ul");s.classList.add("tree-view");const c=document.createElement("li"),i=document.createElement("div");i.style.width="100%",i.style.display="flex",i.style.flexDirection="row";const n=document.createElement("div");n.style.width="20%",n.innerText=r.date,n.style.fontWeight="bold",n.style.fontSize="12px";const l=document.createElement("div");l.style.width="80%",l.innerText=r.value,l.style.textAlign="left",l.style.fontSize="12px";let e;if(m!==d.NONE){const t=document.createElement("img");switch(t.src=m,t.style.width="20px",t.style.height="20px",e=document.createElement("button"),e.classList.add("small-button"),m){case d.ADD:e.onclick=()=>{u.push(r),document.querySelector("[aria-controls='production-studio']").children[0].innerText="Production Studio ("+u.length+")",t.src="../icons/good.png",setTimeout(()=>{t.src="../icons/add.png"},500)};break;case d.REMOVE:e.onclick=()=>{const a=u.findIndex(p=>p===r);u.splice(a,1),document.querySelector("[aria-controls='production-studio']").children[0].innerText="Production Studio ("+u.length+")",s.remove()};break}e.appendChild(t)}i.appendChild(n),i.appendChild(l),m!==d.NONE&&i.appendChild(e),c.appendChild(i),s.appendChild(c),o.appendChild(s)}}
